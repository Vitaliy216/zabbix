...
/usr/bin/mysqladmin -u root -preverse password 'Genbygblfh2016'
mysql -h localhost -uroot -pGenbygblfh2016 -P 3306 -s <<< 'CREATE DATABASE zabbix CHARACTER SET UTF8'
mysql -h localhost -uroot -pGenbygblfh2016 -P 3306 -s <<< 'GRANT ALL PRIVILEGES on zabbix.* to "zabbix"@"localhost" IDENTIFIED BY "Genbygblfh2016"'
groupadd zabbix
...
mysql -uzabbix -pGenbygblfh2016 zabbix < schema.sql
mysql -uzabbix -pGenbygblfh2016 zabbix < images.sql
mysql -uzabbix -pGenbygblfh2016 zabbix < data.sql
cd ~/zabbix-*/
...
sed -i "s/^.*DBPassword=.*$/DBPassword=Genbygblfh2016/" /usr/local/etc/zabbix_server.conf
